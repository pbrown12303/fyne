// Package nohead provides utility drivers for running application tests without requiring a screen.
package nohead

import (
	"net/url"
	"sync"

	"fyne.io/fyne/v2"
	"fyne.io/fyne/v2/internal"
	"fyne.io/fyne/v2/internal/app"
	"fyne.io/fyne/v2/internal/cache"
	"fyne.io/fyne/v2/internal/painter"
	"fyne.io/fyne/v2/theme"
)

// ensure we have a dummy app loaded and ready to test
func init() {
	NewHeadlessApp()
}

// HeadlessApp is an app that has no screen.
type HeadlessApp struct {
	driver       *headlessDriver
	settings     *testSettings
	prefs        fyne.Preferences
	propertyLock sync.RWMutex
	storage      fyne.Storage
	lifecycle    fyne.Lifecycle
	cloud        fyne.CloudProvider

	// user action variables
	appliedTheme     fyne.Theme
	lastNotification *fyne.Notification
}

// Assert compliance with fyne.App
var _ fyne.App = (*HeadlessApp)(nil)

// CloudProvider returns the headless app's cloud provider
func (a *HeadlessApp) CloudProvider() fyne.CloudProvider {
	return a.cloud
}

// Icon returns the app's icon
func (a *HeadlessApp) Icon() fyne.Resource {
	return nil
}

// SetIcon is a noop
func (a *HeadlessApp) SetIcon(fyne.Resource) {
	// no-op
}

// NewWindow asks the driver to create a new window
func (a *HeadlessApp) NewWindow(title string) fyne.Window {
	return a.driver.CreateWindow(title)
}

// OpenURL is a noop
func (a *HeadlessApp) OpenURL(url *url.URL) error {
	// no-op
	return nil
}

// Run is a noop
func (a *HeadlessApp) Run() {
	// no-op
}

// Quit is a noop
func (a *HeadlessApp) Quit() {
	// no-op
}

// UniqueID returns the app's ID
func (a *HeadlessApp) UniqueID() string {
	return "HeadlessApp" // TODO should this be randomised?
}

// Driver returns the app's driver
func (a *HeadlessApp) Driver() fyne.Driver {
	return a.driver
}

// SendNotification sets the lastNotification value for the app
func (a *HeadlessApp) SendNotification(notify *fyne.Notification) {
	a.propertyLock.Lock()
	defer a.propertyLock.Unlock()

	a.lastNotification = notify
}

// SetCloudProvider sets the cloud provider
func (a *HeadlessApp) SetCloudProvider(p fyne.CloudProvider) {
	if p == nil {
		a.cloud = nil
		return
	}

	a.transitionCloud(p)
}

// Settings returns the app's settings
func (a *HeadlessApp) Settings() fyne.Settings {
	return a.settings
}

// Preferences returns the app's preferences
func (a *HeadlessApp) Preferences() fyne.Preferences {
	return a.prefs
}

// Storage returns the app's storage
func (a *HeadlessApp) Storage() fyne.Storage {
	return a.storage
}

// Lifecycle returns the apps lifecycle
func (a *HeadlessApp) Lifecycle() fyne.Lifecycle {
	return a.lifecycle
}

// Metadata returns the app's metadata
func (a *HeadlessApp) Metadata() fyne.AppMetadata {
	return fyne.AppMetadata{} // just dummy data
}

func (a *HeadlessApp) lastAppliedTheme() fyne.Theme {
	a.propertyLock.Lock()
	defer a.propertyLock.Unlock()

	return a.appliedTheme
}

func (a *HeadlessApp) transitionCloud(p fyne.CloudProvider) {
	if a.cloud != nil {
		a.cloud.Cleanup(a)
	}

	err := p.Setup(a)
	if err != nil {
		fyne.LogError("Failed to set up cloud provider "+p.ProviderName(), err)
		return
	}
	a.cloud = p

	listeners := a.prefs.ChangeListeners()
	if pp, ok := p.(fyne.CloudProviderPreferences); ok {
		a.prefs = pp.CloudPreferences(a)
	} else {
		a.prefs = internal.NewInMemoryPreferences()
	}
	if store, ok := p.(fyne.CloudProviderStorage); ok {
		a.storage = store.CloudStorage(a)
	} else {
		a.storage = &headlessStorage{}
	}

	for _, l := range listeners {
		a.prefs.AddChangeListener(l)
		l() // assume that preferences have changed because we replaced the provider
	}

	// after transition ensure settings listener is fired
	a.settings.apply()
}

// NewHeadlessApp returns a new dummy app used for testing.
// It loads a test driver which creates a virtual window in memory for testing.
func NewHeadlessApp() *HeadlessApp {
	settings := &testSettings{scale: 1.0, theme: Theme()}
	prefs := internal.NewInMemoryPreferences()
	store := &headlessStorage{}
	test := &HeadlessApp{settings: settings, prefs: prefs, storage: store, driver: newHeadlessDriver(),
		lifecycle: &app.Lifecycle{}}
	root, _ := store.docRootURI()
	store.Docs = &internal.Docs{RootDocURI: root}
	painter.ClearFontCache()
	cache.ResetThemeCaches()
	fyne.SetCurrentApp(test)

	listener := make(chan fyne.Settings)
	test.Settings().AddChangeListener(listener)
	go func() {
		for {
			<-listener
			test.propertyLock.Lock()
			painter.ClearFontCache()
			cache.ResetThemeCaches()
			app.ApplySettings(test.Settings(), test)

			test.appliedTheme = test.Settings().Theme()
			test.propertyLock.Unlock()
		}
	}()

	return test
}

type testSettings struct {
	theme fyne.Theme
	scale float32

	changeListeners []chan fyne.Settings
	propertyLock    sync.RWMutex
}

func (s *testSettings) AddChangeListener(listener chan fyne.Settings) {
	s.propertyLock.Lock()
	defer s.propertyLock.Unlock()
	s.changeListeners = append(s.changeListeners, listener)
}

func (s *testSettings) BuildType() fyne.BuildType {
	return fyne.BuildStandard
}

func (s *testSettings) PrimaryColor() string {
	return theme.ColorBlue
}

func (s *testSettings) SetTheme(theme fyne.Theme) {
	s.propertyLock.Lock()
	s.theme = theme
	s.propertyLock.Unlock()

	s.apply()
}

func (s *testSettings) ShowAnimations() bool {
	return true
}

func (s *testSettings) Theme() fyne.Theme {
	s.propertyLock.RLock()
	defer s.propertyLock.RUnlock()

	if s.theme == nil {
		return theme.DarkTheme()
	}

	return s.theme
}

func (s *testSettings) ThemeVariant() fyne.ThemeVariant {
	return 2 // not a preference
}

func (s *testSettings) Scale() float32 {
	s.propertyLock.RLock()
	defer s.propertyLock.RUnlock()
	return s.scale
}

func (s *testSettings) apply() {
	s.propertyLock.RLock()
	listeners := s.changeListeners
	s.propertyLock.RUnlock()

	for _, listener := range listeners {
		select {
		case listener <- s:
		default:
			l := listener
			go func() { l <- s }()
		}
	}
}
