...
func buildMenuOverlay(menus *fyne.MainMenu, w *headlessWindow) fyne.CanvasObject {
...
func addMissingQuitForMainMenu(menus *fyne.MainMenu, w *headlessWindow) *fyne.MainMenu {
		...
		if item.IsQuit && item.Action == nil {
			item.Action = func() {
				for _, win := range w.driver.AllWindows() {
					if glWin, ok := win.(*headlessWindow); ok {
