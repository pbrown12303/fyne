...
func (w *headlessWindow) SetFullScreen(full bool) {
	w.fullScreen = full
	if !w.visible {
		return
	}
...
func (w *headlessWindow) SetOnDropped(dropped func(pos fyne.Position, items []fyne.URI)) {
...
func (w *headlessWindow) doCenterOnScreen() {
	viewWidth, viewHeight := w.screenSize(w.canvas.size)
	if w.width > viewWidth { // in case our headlessWindow has not called back to canvas size yet
		viewWidth = w.width
	}
	...
	// get headlessWindow dimensions in pixels
	// monitor := w.getMonitorForWindow()
	...
	// set new headlessWindow coordinates
...
func (w *headlessWindow) SetIcon(icon fyne.Resource) {
	w.icon = icon
	if icon == nil {
	...
	w.runOnMainWhenCreated(func() {
		...
		// 		if err != nil {
		// 			fyne.LogError("Failed to decode image for headlessWindow icon", err)
		// 			return
		// 		}
		// 		img = pix
		// 	}
...
func (w *headlessWindow) fitContent() {
...
func (w *headlessWindow) getMonitorForWindow() *glfw.Monitor {
	x, y := w.xpos, w.ypos
	if w.fullScreen {
...
func (w *headlessWindow) detectScale() float32 {
...
func (w *headlessWindow) moved(_ *glfw.Window, x, y int) {
	w.processMoved(x, y)
}

func (w *headlessWindow) resized(_ *glfw.Window, width, height int) {
	w.processResized(width, height)
}

func (w *headlessWindow) scaled(_ *glfw.Window, x float32, y float32) {
...
func (w *headlessWindow) frameSized(_ *glfw.Window, width, height int) {
	w.processFrameSized(width, height)
}

func (w *headlessWindow) refresh(_ *glfw.Window) {
	w.processRefresh()
}

func (w *headlessWindow) closed(viewport *glfw.Window) {
	if viewport != nil {
...
func (w *headlessWindow) SetCursor(cursor *glfw.Cursor) {
	w.viewport.SetCursor(cursor)
}

func (w *headlessWindow) setCustomCursor(rawCursor *glfw.Cursor, isCustomCursor bool) {
...
func (w *headlessWindow) mouseMoved(_ *glfw.Window, xpos, ypos float64) {
	w.processMouseMoved(xpos, ypos)
}

func (w *headlessWindow) mouseClicked(_ *glfw.Window, btn glfw.MouseButton, action glfw.Action, mods glfw.ModifierKey) {
...
func (w *headlessWindow) mouseScrolled(viewport *glfw.Window, xoff float64, yoff float64) {
...
func (w *headlessWindow) keyPressed(_ *glfw.Window, key glfw.Key, scancode int, action glfw.Action, mods glfw.ModifierKey) {
...
func (w *headlessWindow) charInput(viewport *glfw.Window, char rune) {
	w.processCharInput(char)
}

func (w *headlessWindow) focused(_ *glfw.Window, focused bool) {
	w.processFocused(focused)
}

func (w *headlessWindow) DetachCurrentContext() {
	glfw.DetachCurrentContext()
}

func (w *headlessWindow) rescaleOnMain() {
	if w.isClosing() {
		return
	}
	w.fitContent()
...
func (w *headlessWindow) create() {
	runOnMain(func() {
		...
		if err != nil {
			w.driver.initFailed("headlessWindow creation error", err)
			return
		}
	...
	runOnMain(func() {
		...
		// update headlessWindow size now we have scaled detected
		w.fitContent()
...
func (w *headlessWindow) view() *glfw.Window {
	w.viewLock.RLock()
