...
// minSizeOnScreen gets the padded minimum size of a headlessWindow content in screen pixels
func (w *headlessWindow) minSizeOnScreen() (int, int) {
	// get minimum size of content inside the headlessWindow
...
func (w *headlessWindow) screenSize(canvasSize fyne.Size) (int, int) {
...
func (w *headlessWindow) Resize(size fyne.Size) {
	// we cannot perform this until headlessWindow is prepared as we don't know it's scale!
...
func (w *headlessWindow) calculatedScale() float32 {
	return calculateScale(userScale(), fyne.CurrentDevice().SystemScaleForWindow(w), w.detectScale())
}

func (w *headlessWindow) detectTextureScale() float32 {
...
func (w *headlessWindow) Close() {
	...
	// trigger callbacks - early so headlessWindow still exists
	if w.onClosed != nil {
...
func (w *headlessWindow) Clipboard() fyne.Clipboard {
	...
	if w.clipboard == nil {
		w.clipboard = &headlessClipboard{headlessWindow: w.viewport}
	}
	return w.clipboard
}
...
func (w *headlessWindow) SetContent(content fyne.CanvasObject) {
	w.viewLock.RLock()
	visible := w.visible
	w.viewLock.RUnlock()
...
func (w *headlessWindow) processClosed() {
...
// destroy this headlessWindow and, if it's the last headlessWindow quit the app
func (w *headlessWindow) destroy(d *gLDriver) {
	w.DestroyEventQueue()
...
func (w *headlessWindow) processMoved(x, y int) {
...
func (w *headlessWindow) processResized(width, height int) {
...
func (w *headlessWindow) processFrameSized(width, height int) {
...
func (w *headlessWindow) processRefresh() {
	refreshWindow(w)
}

func (w *headlessWindow) findObjectAtPositionMatching(canvas *glCanvas, mouse fyne.Position, matches func(object fyne.CanvasObject) bool) (fyne.CanvasObject, fyne.Position, int) {
	return driver.FindObjectAtPositionMatching(mouse, matches, canvas.Overlays().Top(), canvas.menu, canvas.Content())
}

func (w *headlessWindow) processMouseMoved(xpos float64, ypos float64) {
	w.mouseLock.Lock()
...
func (w *headlessWindow) objIsDragged(obj interface{}) bool {
...
func (w *headlessWindow) mouseIn(obj desktop.Hoverable, ev *desktop.MouseEvent) {
	w.QueueEvent(func() {
		if obj != nil {
			obj.MouseIn(ev)
		}
...
func (w *headlessWindow) mouseOut() {
	w.QueueEvent(func() {
		w.mouseLock.RLock()
		mouseOver := w.mouseOver
		w.mouseLock.RUnlock()
...
func (w *headlessWindow) processMouseClicked(button desktop.MouseButton, action action, modifiers fyne.KeyModifier) {
	w.mouseLock.RLock()
	w.mouseDragPos = w.mousePos
	mousePos := w.mousePos
	mouseDragStarted := w.mouseDragStarted
	w.mouseLock.RUnlock()
	if mousePos.IsZero() { // headlessWindow may not be focused (darwin mostly) and so position callbacks not happening
...
func (w *headlessWindow) mouseClickedHandleMouseable(mev *desktop.MouseEvent, action action, wid desktop.Mouseable) {
...
func (w *headlessWindow) mouseClickedHandleTapDoubleTap(co fyne.CanvasObject, ev *fyne.PointEvent) {
...
func (w *headlessWindow) waitForDoubleTap(co fyne.CanvasObject, ev *fyne.PointEvent) {
...
func (w *headlessWindow) processMouseScrolled(xoff float64, yoff float64) {
	w.mouseLock.RLock()
	mousePos := w.mousePos
	w.mouseLock.RUnlock()
	co, pos, _ := w.findObjectAtPositionMatching(w.canvas, mousePos, func(object fyne.CanvasObject) bool {
...
func (w *headlessWindow) capturesTab(modifier fyne.KeyModifier) bool {
	captures := false
...
func (w *headlessWindow) processKeyPressed(keyName fyne.KeyName, keyASCII fyne.KeyName, scancode int, action action, keyDesktopModifier fyne.KeyModifier) {
...
func (w *headlessWindow) processCharInput(char rune) {
...
func (w *headlessWindow) processFocused(focus bool) {
	if focus {
		if curWindow == nil {
		...
		go func() { // check whether another headlessWindow was focused or not
			time.Sleep(time.Millisecond * 100)
			if curWindow != w {
				return
			}
...
func (w *headlessWindow) triggersShortcut(localizedKeyName fyne.KeyName, key fyne.KeyName, modifier fyne.KeyModifier) bool {
...
func (w *headlessWindow) RunWithContext(f func()) {
	if w.isClosing() {
		return
	}
...
func (w *headlessWindow) RescaleContext() {
	runOnMain(w.rescaleOnMain)
}

func (w *headlessWindow) Context() interface{} {
	return nil
}

func (w *headlessWindow) runOnMainWhenCreated(fn func()) {
	if w.view() != nil {
		runOnMain(fn)
		return
	}
...
func (d *gLDriver) createWindow(title string, decorate bool) fyne.Window {
	...
	runOnMain(func() {
		...
		ret = &headlessWindow{title: title, decorate: decorate, driver: d}
		// This queue is destroyed when the headlessWindow is closed.
		ret.InitEventQueue()
...
func (w *headlessWindow) doShowAgain() {
	if w.isClosing() {
		return
	}
